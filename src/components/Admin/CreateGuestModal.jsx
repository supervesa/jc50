...
import { Save, X, UserPlus } from 'lucide-react'; // UUDET IKONIT
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="jc-card modal-card" onClick={e => e.stopPropagation()}>
        ...
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'1.5rem'}}>
          ...
          <button onClick={onClose} className="btn-back" style={{fontSize:'1.5rem', lineHeight:1}}>
            <X size={24} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="jc-form" style={{maxWidth:'100%'}}>
          ...
          <div className="jc-field">
            <label>Nimi *</label>
            <input required type="text" name="name" placeholder="Etu- ja sukunimi" value={formData.name} onChange={handleChange} className="jc-input-custom" />
          </div>

          <div className="jc-field">
            <label>Sähköposti *</label>
            <input required type="email" name="email" placeholder="osoite@esimerkki.fi" value={formData.email} onChange={handleChange} className="jc-input-custom" />
          </div>
          ...
          <div className="jc-field">
            ...
            <textarea name="dietary_restrictions" value={formData.dietary_restrictions} onChange={handleChange} className="jc-input-custom" rows="3" />
          </div>

          <div style={{display:'flex', gap:'10px', marginTop:'1rem'}}>
            <button type="button" onClick={onClose} className="jc-cta ghost" style={{flex:1}}>Peruuta</button>
            <button type="submit" className="jc-cta primary" style={{flex:2, display:'flex', alignItems:'center', justifyContent:'center', gap:'8px'}} disabled={loading}>
