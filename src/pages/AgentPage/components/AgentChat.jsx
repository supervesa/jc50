...
const AgentChat = ({ 
  guestId, chatHistory, characterMap, activePoll, hasVoted, onVote, onSend 
}) => {
          ...
          {hasVoted ? (
            ...
            <div className="ap-poll-options">
              {activePoll.options.map((opt, i) => (
                <button key={i} className="ap-poll-btn" onClick={() => onVote(i)}>{opt}</button>
              ))}
            </div>
          )}
        </div>
            ...
            <div key={msg.id} className={`chat-row ${isMe ? 'row-me' : 'row-other'}`}>
              {!isMe && (
                <div className="chat-avatar-thumb">
                  {avatarUrl ? <img src={avatarUrl} alt="" /> : <div className="chat-avatar-initial">{initial}</div>}
                </div>
              )}
