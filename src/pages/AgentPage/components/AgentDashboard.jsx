...
const AgentDashboard = ({ 
  ...
  return (
    // PÄÄKONTTI: Tämä täyttää koko käytettävissä olevan tilan (flex-grow: 1 AgentPagen sisällä)
    // overflow-hidden estää koko sivun rullaamisen, pakotamme rullauksen vain sisällölle.
    <div className="dashboard-scroll-wrapper" style={{ 
      display: 'flex', 
      ...
      {/* SKROLLATTAVA ALUE: Tämä on se "ikkuna", joka liikkuu */}
      <div className="dashboard-content" style={{ 
        flex: 1, 
        overflowY: 'auto', 
        WebkitOverflowScrolling: 'touch', // Pehmeä skrollaus iOS
        paddingBottom: 'calc(120px + env(safe-area-inset-bottom))' // TÄRKEÄ: Turvaväli alhaalla
      }}>

        {/* HUD-PALKKI (Sticky Header) */}
        <div className="compact-hud" style={{
          position: 'sticky', // TÄMÄ PITÄÄ PALKIN YLHÄÄLLÄ
          top: 0,
          zIndex: 100,
          display: 'flex',
          alignItems: 'center',
          background: 'rgba(22, 22, 22, 0.95)', // Hieman läpinäkyvä
          backdropFilter: 'blur(10px)', // iOS blur efekti
        ...
        {/* ÄÄNESTYS (Nyt se on osa normaalia virtaa ja rullautuu kauniisti) */}
        {activePoll && (
          <div style={{ margin: '0 15px 25px 15px' }}>
            <DashboardPoll poll={activePoll} hasVoted={hasVoted} onVote={onVote} />
          </div>
        )}
        ...
        {missionsOpen ? (
          ...
          <>
            ...
            <div className="dash-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', padding: '0 15px', marginBottom: '25px' }}>
        ...
        {/* CHAT-NAPPI (Viimeisenä, mutta turvavälin ansiosta ei jää piiloon) */}
