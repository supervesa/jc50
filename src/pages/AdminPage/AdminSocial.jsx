...
const AdminSocial = ({ characters }) => {
  ...
  // UUSI TILA: Keskitetyt säännöt
  const [xpConfig, setXpConfig] = useState(null);

  useEffect(() => {
    const fetchRules = async () => {
      const { data } = await supabase.from('game_rules').select('value').eq('rule_key', 'xp_config').single();
  ...
  const rewardAvatars = async () => {
    ...
    const rewardAmount = xpConfig?.avatar_bonus || 50; // Dynaaminen arvo
  ...
  const stats = useMemo(() => {
    ...
    posts.forEach(p => {
      ...
      if (!map[p.guest_id]) {
        map[p.guest_id] = { id: p.guest_id, name: p.sender_name || 'Tuntematon', count: 0, hotSum: 0, bestImg: null, maxHot: -1 };
      }
    ...
    return { 
      byCount: [...arr].sort((a, b) => b.count - a.count).slice(0, 5),
      byHot: [...arr].sort((a, b) => b.hotSum - a.hotSum).slice(0, 5) 
    };
  }, [posts]);
    ...
    <div className="social-admin-wrapper">
      <div className="avatar-campaign-box">
        <div className="ac-info">
          <h3>📸 PROFILOINTI</h3>
          <p>{avatarCandidates.length > 0 ? `Löytyi ${avatarCandidates.length} uutta kuvaa!` : "Kaikki kuvalliset on palkittu."}</p>
        </div>
        <button 
        ...
        >
          {loadingAvatar ? '...' : `🚀 PALKITSE (${avatarCandidates.length * (xpConfig?.avatar_bonus || 50)} XP)`}
        </button>
      </div>
      ...
      <div className="social-stats-box">
        <div className="social-header">
          ...
          <div className="time-selector">
            {['15', '30', '60', 'all'].map(t => (
              <button key={t} className={timeWindow === t ? 'active' : ''} onClick={() => setTimeWindow(t)}>{t === 'all' ? 'KOKO ILTA' : `${t} min`}</button>
            ))}
          </div>
        </div>
          ...
          <div className="stat-col">
            ...
            {stats.byCount.map(user => (
              <div key={'c' + user.id} className={`stat-row ${selectedSocialIds.has(user.id) ? 'selected' : ''}`} onClick={() => toggleSocialSelect(user.id)}>
                ...
                <div className="stat-info"><span className="name">{user.name}</span><span className="score">{user.count} kuvaa</span></div>
              </div>
            ))}
             {stats.byCount.length > 0 && (
              <button className="btn-mini-reward" onClick={() => giveSocialBonus(xpConfig?.social_active || 10, '📸 Aktiivinen kuvaaja')}>
                Palkitse (+{xpConfig?.social_active || 10})
              </button>
            )}
          </div>
          ...
          <div className="stat-col">
            ...
            {stats.byHot.map(user => (
              <div key={'h' + user.id} className={`stat-row ${selectedSocialIds.has(user.id) ? 'selected' : ''}`} onClick={() => toggleSocialSelect(user.id)}>
                ...
                <div className="stat-info"><span className="name">{user.name}</span><span className="score">🔥 {user.hotSum}</span></div>
              </div>
            ))}
            {stats.byHot.length > 0 && (
              <button className="btn-mini-reward" onClick={() => giveSocialBonus(xpConfig?.social_hot || 25, '🔥 Sometykki (Hot Content)')}>
                Palkitse (+{xpConfig?.social_hot || 25})
              </button>
            )}
          </div>
        </div>
